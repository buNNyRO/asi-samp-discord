		...
		if (data.connect == SAMP::SAMP_CONNECT_SERVER) {
			...
			while (true) {
				...
				if (query.info(information)) {
					auto fullAddress = data.address + ':' + data.port + " (" + std::to_string(information.basic.players) + "/" + std::to_string(information.basic.maxPlayers) + ")";
					auto info = "Playing as " + data.username + " in " + information.language;
					auto players = std::to_string(information.basic.players) + "/" + std::to_string(information.basic.maxPlayers);
					auto image = logo;
					if (image == "logo") {
